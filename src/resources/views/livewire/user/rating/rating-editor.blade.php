    ...
    <div x-data="{
        ...
        <div class="flex justify-center mt-8 md:justify-end gap-x-5">
            <div class="w-32 space-y-1">
                <div class="h-3 w-full items-center grid-flow-col grid gap-1
                    grid-cols-{{ $player['momLimit'] }}">
                    @foreach($remainingMomCountRange as $count)
                        <x-svg.remaining-count class="fill-amber-300 w-5" />
                    @endforeach

                    @foreach($momCountRange as $count)
                        <x-svg.count class="w-5" />
                    @endforeach
                </div>

                <button class="w-full py-1 border-2 border-gray-200 rounded-lg bg-amber-400"
            ...
            <div class="w-32 space-y-1">
                <div class="h-3 w-full items-center grid-flow-col grid gap-1 grid-cols-{{ $player['rateLimit'] }}">
                    @foreach($remainingRateCountRange as $count)
                        <x-svg.remaining-count class="fill-sky-500 w-10" />
                    @endforeach

                    @foreach($rateCountRange as $count)
                        <x-svg.count class="w-10" />
                    @endforeach
                </div>

                <button class="w-full py-1 border-2 border-gray-200 rounded-lg pointer-events-none opacity-30 bg-sky-600"
