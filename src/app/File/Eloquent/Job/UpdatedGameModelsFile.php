...
class UpdatedGameModelsFile extends FileHandler implements PathInterface
{
    ...
    public function write()
    {
        ...
        $game3 = Game::whereFixtureId(1299338)->first();
        $game3->is_end = false;
        $game3->is_winner = null;
        $game3->is_details_fetched = false;
        $game3->started_at = Carbon::create(2024, 11, 11, 0, 2, 0, 'UTC')->format('Y-m-d H:i:s');
        $game3->finished_at = Carbon::create(2024, 11, 11, 0, 2, 0, 'UTC')->addMinutes(1)->format('Y-m-d H:i:s');

        $data = collect()->push($game)->push($game2)->push($game3);
    ...
    {
        return app_path(self::DIR_PATH.'/'.'updated-games'.self::EXTENSION);
    }
}